...
A local workspace marker is recovery data, not reconnect consent. Start/Join creates a non-secret, one-use handoff bound to the exact session, participant, working folder, and current VS Code main process. Pair Notebook remains loaded in Restricted Mode only to hold that explicit handoff; it starts no runtime, signalling, synchronization, or execution until Workspace Trust is granted. After Trust, that already explicit action may continue automatically once. A restart, reinstall, unrelated marker, or ordinary activation has no matching handoff and only offers a manual connection action. Suspend detection is armed only after the same fully started runtime has been observed across timer ticks, so Trust/reload/startup gaps cannot cause a local leave. Graceful extension shutdown and a detected system-suspend gap perform a local leave and update the Recent Session host/exit metadata while retaining marker and SecretStorage state for manual recovery. Recent Session presentation includes the host display name, relative exit age, and local `dd/mm/yy` date.

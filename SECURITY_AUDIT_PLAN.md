...
### SEC-008 — High — Resolved — Audited fixes are absent from the public and installed 0.5.5 release
...
**Pre-release verification:** The 0.5.6 candidate passes lint and 299 TypeScript/integration tests, including the real Jupyter-kernel case; the Python bridge passes 7 unit tests. Production-only and full `npm audit` both report zero vulnerabilities. Artifact preflight accepts 92 packageable files, the VSIX contains 16 validated entries, and the source ZIP contains 93 entries under one top-level directory with no sensitive or unexpected nested archives. The local VSIX SHA-256 is `dfe3a7baa573f91c2f2b6a50caf071392c8a6bd5667add02734ceab58009e5a3`; the source ZIP SHA-256 is `d2dd068dfbd99cc5dd6248e4c4479f27ca42123fcb478730936ee4ecddc8375b`. The local, VSIX, and ZIP bundles are byte-identical at SHA-256 `bb41020f5e497c18fa59e3354722af2dac1a458c7b79bbca545e486d4f9d5d17` and contain the signed relay-envelope v2, protocol-v4, session-scoped compute, and proxy SecretStorage markers. Public Trystero/Nostr/WebRTC and MQTT/WebRTC smokes pass. The first forced Nostr-only emergency-relay attempt timed out without a protocol error; an immediate clean rerun passed all seven Nostr/MQTT/redundant combinations with bidirectional 8 × 64 KiB bursts. Public tag, release asset, and installed-extension verification remain pending and are required before this finding can be resolved.

**Resolution:** Resolved by the public [v0.5.6 release](https://github.com/r7zex/jupyter_pair_ext/releases/tag/v0.5.6). Annotated tag object `6aded456ef9eae6a136c48cbce7f22d53fc99f08` dereferences to verified commit `3afe7bdd02e99de1775e12376ade5b0b0e9d1857`. [GitHub Actions run 33324562378](https://github.com/r7zex/jupyter_pair_ext/actions/runs/33324562378) completed both the read-only verification job and the separately authorized publication job successfully. At publication time, GitHub recorded SHA-256 `f958deb447002fc5161b752a3b75aaef891f9544d3148f3e8e0fb9c12368fd5a` for both VSIX names, `6a8fa2409ad2002fd3211ed35db4cc6ce20cbecd5feb807e1a6a4e33c398737a` for the source ZIP, and `21d0e63c1d22a3608c3f75e01ce02dfadc8453bed60cc846452d691bf05ee50b` for `SHA256SUMS.txt`. A separately downloaded verified workflow artifact matches all four historical public digests; its stable and versioned VSIX files are identical, its 16-entry VSIX and 93-entry source ZIP pass the sensitive/nested-archive checks, and all three copies of `out/extension.js` match the locally verified bundle at SHA-256 `bb41020f5e497c18fa59e3354722af2dac1a458c7b79bbca545e486d4f9d5d17`. After a usability review, the public release was simplified to the single versioned VSIX; the removed stable alias, source ZIP, and checksum file remain recorded here as historical publication evidence and in the successful workflow artifact. VS Code reports `pair-notebook.pair-notebook@0.5.6` installed at `C:\Users\User\.vscode\extensions\pair-notebook.pair-notebook-0.5.6`; its bundle and Python bridge are byte-identical to the verified GitHub artifact, no `node_modules` directory is present, and the installed bundle contains the relay-envelope v2, protocol-v4, session-scoped compute, and proxy SecretStorage markers. Independently rebuilt Windows and Linux archive container hashes differ, while their runtime bundle bytes are identical; the Python bridge differs locally only by CRLF/LF normalization and is byte-identical across the public VSIX, public source ZIP, and installed extension. Direct release-CDN download from this host **FAILED / NOT VERIFIED** after connection timeouts to all four published CDN IPv4 addresses; public asset integrity was instead proven by matching GitHub's recorded digests to the independently downloaded workflow artifact that the successful publication job consumed. CDN reachability from this host is therefore not claimed.

...
Text authority remains with the current Session Host. Guests edit optimistic local replicas and send immutable revision/digest-bound intents; the host serializes accepted operations and returns acknowledgements or compact state-vector repairs. If the pinned host is temporarily unreachable, guest intents remain in a bounded queue instead of becoming independent canonical CRDT updates. Participant line highlights are advisory and never reject or roll back text.
...
Each participant also owns an Ed25519 identity key. The private key remains in VS Code SecretStorage; the invite pins the host public key, and authenticated peer keys remain pinned across disconnects and voluntary transfer. This prevents another invite holder from impersonating a known offline participant. Protocol v6, first released in Pair Notebook 0.5.20, retains the signed emergency-relay envelope and host-canonical execution framing from v5 and makes host-authoritative text intents an admission boundary. It intentionally rejects v5 and older clients; all participants in a session must run Pair Notebook 0.5.20 or later.

...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function render() {
      ...
      const pauseAction = paused && state.isHost ? '<div class="actions">'+action('backingFolder','Настроить папку нового хоста','ПРОДОЛЖИТЬ',{secondary:false,wide:true})+'</div>' : '';
      const pauseNotice = paused ? '<div class="card"><strong>Совместная сессия приостановлена</strong><div class="tiny">'+esc(state.isHost?'Выберите пустую папку или подключите проверенную общую папку с файлами. Если закрыть диалог, эта кнопка останется доступна.':'Новый хост выбирает папку на своём компьютере. Дождитесь продолжения сессии.')+'</div>'+pauseAction+'</div>' : '';

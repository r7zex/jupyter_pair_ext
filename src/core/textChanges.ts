...
import { TextChange } from './crdt';

/** Rebases immutable edits from one text baseline onto another. */
export function rebaseTextChanges(before: string, target: string, changes: readonly TextChange[]): TextChange[] {
  if (before === target) return [...changes];
  // Bound reconciliation work for large, unrelated documents. The fallback
  // preserves new typing at the end without deleting authoritative content.
  ...
  const add = (offset: number, deleteCount: number, insertText: string): void => {
    ...
    result.set(offset, {
      offset,
      deleteCount: (previous?.deleteCount ?? 0) + deleteCount,
